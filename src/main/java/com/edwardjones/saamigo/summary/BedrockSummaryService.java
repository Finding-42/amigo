...
import com.edwardjones.saamigo.summary.domain.AnthropicClaudeParameters;
import com.edwardjones.saamigo.summary.domain.CohereCommandParameters;
...
public class BedrockSummaryService {
    ...
    public Mono<String> summarizeText(@NonNull String text, BedrockModel model) {
        return Mono.fromCallable(() -> {
            ...
            return switch (model) {
                case COHERE_COMMAND -> mapper.writeValueAsString(CohereCommandParameters.usingPrompt(text));
                case CLAUDE_V1_INSTANT -> mapper.writeValueAsString(AnthropicClaudeParameters.usingPrompt(text));
            };
        })
